...
def example_1_basic_config():
    ...
    config = TrainingConfig.quick_start(
        dataset_path="/path/to/dataset", architecture="resnet50", num_classes=4, epochs=50
    )
...
def example_3_save_load_config():
    ...
    # Create config
    config = TrainingConfig.quick_start(
        dataset_path="/path/to/dataset", architecture="resnet50", num_classes=4
    )
...
def example_4_utilities():
    ...
    # Setup file manager
    file_manager = FileManager(base_dir="experiments", experiment_name="test_experiment")
