...
class CoffeeBeanPipeline:
    ...
    def _train_models(self, models_to_train: Optional[List[str]]) -> Dict[str, Any]:
        """Train models."""
        logger.info("\n[3/7] Training models...")
        ...
        training_results = self.trainer.train_all_models(models_to_train, continue_on_error=True)
    ...
    def _evaluate_models(self, training_results: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        ...
        for model_name in self.results["models_trained"]:
            ...
            if checkpoint_path.exists():
                ...
                # Store results
                evaluation_results[model_name] = {"metrics": metrics, "evaluator": evaluator}

                logger.info(f"    Accuracy: {metrics['accuracy']:.4f}")
            else:
                logger.warning(f"    Checkpoint not found for {model_name}")

        self.results["evaluations"] = {k: v["metrics"] for k, v in evaluation_results.items()}

        logger.info(f"✓ Evaluated {len(evaluation_results)} models")
        return evaluation_results

    def _compare_models(self, evaluation_results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        """Compare all models."""
        ...
        return {"comparator": comparator, "best_model": best_model, "best_value": best_value}

    def _register_models(
        self, evaluation_results: Dict[str, Dict[str, Any]], comparison_results: Dict[str, Any]
    ) -> None:
        ...
        for model_name, results in evaluation_results.items():
            ...
            if checkpoint_path.exists():
                ...
                metadata = ModelMetadata(
                    ...
                    input_shape=(
                        self.config.model_config.input_shape
    ...
    def _generate_visualizations(self, evaluation_results: Dict[str, Dict[str, Any]]) -> None:
        ...
        for model_name, results in evaluation_results.items():
            ...
            try:
                ...
                # ROC curves
                evaluator.plot_roc_curves(save_path=str(model_viz_path / "roc_curves.png"))

                # Generate report
                evaluator.generate_report(save_path=str(model_viz_path / "evaluation_report.txt"))
