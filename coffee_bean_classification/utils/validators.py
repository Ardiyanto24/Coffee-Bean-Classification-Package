...
def validate_shape(
    ...
    if ndim is not None and array.ndim != ndim:
        raise ValidationError(
            f"Expected {ndim} dimensions, got {array.ndim}"
        )
    
    if expected_shape is not None:
        if len(expected_shape) != array.ndim:
            raise ValidationError(
                f"Shape mismatch: expected {len(expected_shape)} dims, "
                f"got {array.ndim} dims"
            )
...
def validate_ratio(ratio: Union[float, Tuple[float, ...]], total: float = 1.0) -> bool:
    ...
    if isinstance(ratio, (tuple, list)):
        ...
        if not np.isclose(ratio_sum, total):
            raise ValidationError(
                f"Ratios sum to {ratio_sum}, expected {total}"
            )
        ...
        if ratio < 0 or ratio > total:
            raise ValidationError(
                f"Ratio {ratio} not in valid range [0, {total}]"
            )
...
def validate_image_size(size: Union[int, Tuple[int, int]]) -> Tuple[int, int]:
    ...
    if isinstance(size, int):
        ...
        raise ValidationError(
            f"Image size must be int or (height, width) tuple, got {size}"
        )
